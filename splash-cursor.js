/* ============================================================
   EVEREST — SplashCursor (WebGL fluid)
   Vanilla port of the React Bits <SplashCursor /> component.
   Brand-tuned: RAINBOW_MODE off, gold dye, scoped to the
   #manifesto section only. Runs the simulation solely while the
   section is on screen; disabled for touch / reduced-motion / no-WebGL.
   ============================================================ */
(function () {
  'use strict';

  const canvas = document.getElementById('manifestoFluid');
  const section = document.getElementById('manifesto');
  if (!canvas || !section) return;

  const reduced = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
  const fine = window.matchMedia('(hover:hover) and (pointer:fine)').matches;
  if (reduced || !fine) { canvas.style.display = 'none'; return; }
  ...
  function scaleByPixelRatio(input) { const pixelRatio = Math.min(window.devicePixelRatio || 1, 2); return Math.floor(input * pixelRatio); }
  ...
  /* ---- pointer input (scoped to the section bounds) ---- */
  function pointInSection(clientX, clientY) {
    const r = canvas.getBoundingClientRect();
    return clientX >= r.left && clientX <= r.right && clientY >= r.top && clientY <= r.bottom;
  }
  function relPos(clientX, clientY) {
    const r = canvas.getBoundingClientRect();
    return { x: scaleByPixelRatio(clientX - r.left), y: scaleByPixelRatio(clientY - r.top) };
  }

  let firstMove = false;
  window.addEventListener('mousemove', (e) => {
    if (!running || !pointInSection(e.clientX, e.clientY)) return;
  ...
  /* ---- run loop, gated by section visibility ---- */
  let running = false;
  let rafId = null;
  ...
  function start() { if (!running) { running = true; lastUpdateTime = Date.now(); rafId = requestAnimationFrame(updateFrame); } }
